...
int main(int argc, char *argv[])
{
    ...
    {
        printf("\nThe target file size is larger than the key size\n");
        exit(1);
    }
    ...
    struct sockaddr_in encode; //set up socket connection to otp_dec_d
    encode.sin_family = AF_INET;
    encode.sin_port = htons(port_num);
    encode.sin_addr.s_addr = INADDR_ANY;

    if (connect(socket_fd,(struct sockaddr *) &encode, sizeof(encode)) == -1)//connect to otp_dec_d
    {
    ...
    int text;
    //printf("\nSending out: %s\n", output); //FOR TESTING
    text = write(socket_fd,output,output_size);//send string containing text and key data

    free(output);
    char *receive;
    receive =  malloc(sizeof (char*) * char_count + 5 );
    text = read(socket_fd,receive,char_count+2);//receive back decoded message
    printf("%s", receive);
    close(text);
    free(receive);
    return 0;
