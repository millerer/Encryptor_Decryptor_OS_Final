 ...
 * Deamon Decoding Program. Receives in a string of encoded text and a key,
 * splits the string into its component parts, decodes the encoded text,
 * and sends the results back to otp_dec
 */
...
typedef struct dynamic_ //dynamic array structure
{
    char *text;
    int size;
}dynamic;
...
void decode(char *encrypt, char *plaintext, char *keytext);//creates a decryption of the user message via the key
...
int main(int argc, char *argv[] )
{
    ...
    struct sockaddr_in server; //set up the port 'server' on user specified address
   ...
   // memcpy(&server.sin_addr, OSU_ip->h_addr, OSU_ip->h_length);
    ...
    listen(socket_fd, 5);//listen for input from otp_dec
    ...
    while (1)
    {
            ...
            {
                close(client_text);//close the input
...
void start_fork(int client_text)
{
    ...
    int encoded; //flags that recived text is encoded
    dynamic *text_arr;
    ...
    while(endpoint == 0)//while the end of the streaming data (marked with '!') has not yet been reached
            {
                ...
                {   if (valid_char(receive[i]) == 0) //copy over valid chars streamed into the program in text_arr
                    ...
                    else if(receive[i] == '+')//if the text is  encrypted, mark as encoded
                    {
                        encoded = 1;
                    }
                }
            ...
            decode(encrypt,plaintext,key);//decode input
            result = write(client_text,encrypt, sub_text_arr_size);//return decoded text to otp_dec
            free(key);
            free(plaintext);
            free(encrypt);
            free(text_arr->text);
...
void decode(char *encrypt, char *plaintext, char *keytext)
{
    int end;
